...
public class Part2 {
	...
	public void findShortestPath(Point start, Point end) throws NoSuchAlgorithmException {
		...
		while (!q.isEmpty()) {
			...
			if (currNode.pos.x == 3 && currNode.pos.y == 3) {
				if (currNode.input.length() > longestPath.length()) {
					longestPath = currNode.input;
				}
				continue;
			}
		...
		System.out.println("Longest path is: " + (longestPath.length()-input.length()));
	}
	...
	public static void main(String[] args) throws NoSuchAlgorithmException {
		new Part2().findShortestPath(new Point(0,0), new Point(3,3));
	}
